...
# The state HISTORY.md's Δ columns compare against, as a history/<stem>.json
# filename stem. Keep in step with the results/baseline.json symlink target
# when the regression-gate baseline moves to a new release.
HISTORY_BASELINE_STATE = "v4.12.0"
...
def phase_b() -> Path:
    ...
    baseline = by_stem.get(HISTORY_BASELINE_STATE)
    if baseline is None:
        print(f"error: baseline state {HISTORY_BASELINE_STATE!r} not found in {HISTORY_DIR} "
              f"(have: {', '.join(sorted(by_stem))})", file=sys.stderr)
        sys.exit(1)
    ...
    lines.append(
        ...
        f"`{baseline_label}` (the active baseline state, `history/{HISTORY_BASELINE_STATE}.json`).\n"
    )
    lines.append(
